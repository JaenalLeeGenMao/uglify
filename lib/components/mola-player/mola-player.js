...
var _errorUtil = require("@source/lib/errorUtil");
...
var Player = /*#__PURE__*/function (_Component) {
  ...
  function Player() {
    ...
    _defineProperty(_assertThisInitialized(_this), "handleOnAdsImpression", function (activeAds) {
      if (activeAds) {
        fetch(activeAds.impression).then(function (res) {
    ...
    _defineProperty(_assertThisInitialized(_this), "initPlayer", /*#__PURE__*/_asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee2() {
      var drm, streamSourceUrl, watchTime, video, player, isSafari, startTime, drmStreamUrl, manifestUri, deviceId, config, req, cert, _config, _config2;

      return regeneratorRuntime.wrap(function _callee2$(_context2) {
        while (1) {
          switch (_context2.prev = _context2.next) {
            case 0:
              drm = (0, _get2["default"])(_this.props, 'drm', ''), streamSourceUrl = (0, _get2["default"])(_this.props, 'streamSourceUrl', ''), watchTime = (0, _get2["default"])(_this.player, 'totalWatchTime', Number((0, _get2["default"])(_this.props, 'watchTimePosition', 0)));
              /** setelah preroll selesai kembali ke totalWatchTime */
              // console.log(watchTime, this.player)

              video = document.getElementById('video-main');
              player = _this.player || new shaka.Player(video); // window.player = player
              // console.log(streamSourceUrl, this.props)

              isSafari = /.*Version.*Safari.*/.test(navigator.userAgent);
              startTime = watchTime > 0 && !isNaN(watchTime) ? watchTime : 0;
      ...
      if (video) {
        ...
        try {
          var _loop = function _loop() {
            ...
            var track = video.addTextTrack('metadata', lngMap[s.label.toLowerCase()], s.label); // track.mode = 'showing'
    ...
    _defineProperty(_assertThisInitialized(_this), "handleSubtreeOnChange", function () {
      var videoHeight = (0, _get2["default"])(document.getElementById('video-main'), 'offsetHeight', '');
    ...
    _defineProperty(_assertThisInitialized(_this), "getRecommendationData", function () {
      var recomData = (0, _get2["default"])(_this.props, 'recommendation.data', []);

      if (recomData && recomData.length > 0) {
        if (recomData[0].id !== _this.props.videoId) {
          return recomData[0];
  ...
  _createClass(Player, [{
    ...
    value: function render() {
      ...
      var _this$props = this.props,
          ...
          playerConfig = _this$props.playerConfig; // console.log('banner', banner)
      ...
      var player = this.player,
          video = player && player.getMediaElement();
      var canStartNextVideo = this.player && !this.player.isPreroll && !this.player.isLive() && video && video.duration - video.currentTime <= 10;
      ...
      }, playerConfig); // console.log(this.state)


      this.handleSubtreeOnChange();
      var videoOnlyHeight = (0, _get2["default"])(document.getElementById('video-context'), 'offsetHeight', 0) - (0, _get2["default"])(document.getElementById('video-banner'), 'height', 0);
      var recomData = this.getRecommendationData();
      return /*#__PURE__*/_react["default"].createElement("div", {
        ref: function ref(node) {
          return _this2.childRefs = node;
        },
        id: "video-context",
        className: _molaPlayer.container
