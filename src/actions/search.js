...
export const getSearchResult = searchText => dispatch => {
  ...
  Dexie.exists('search-cache-database').then(function(exists) {
    if (exists) {
      searchDb
        .transaction(
          'r',
        ...
        .then(() => {
          if (!isExist) {
            ...
            return Common.getSearchResult({ q: searchText }).then(result => {
...
export const getSearchGenre = () => dispatch => {
  ...
  return Common.getSearchGenre().then(result => {
    if (result.meta.status === 'error') {
      dispatch({
...
export const getRecentSearch = sessionId => dispatch => {
  ...
  return Common.getRecentSearch(sessionId).then(result => {
    if (result.meta.status === 'error') {
      dispatch({
